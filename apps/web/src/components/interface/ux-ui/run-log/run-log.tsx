    ...
    <div className="min-w-0">
      ...
      >
        {running ? (
          ...
          <Icon className={cn("h-3 w-3 shrink-0", TONE[step.tone])} />
        )}
...
export function RunLog({ fixture, title }: { fixture: RunFixture; title: string }) {
  ...
  return (
    <section className="flex min-w-0 flex-col rounded-lg border border-[var(--border-color)] bg-[var(--bg-primary)]">
      <header className="flex flex-wrap items-center gap-x-3 gap-y-2 border-b border-[var(--border-color)] px-3 py-2">
        <div className="flex min-w-0 items-center gap-2">
          <h2 className="shrink-0 text-[13px] font-semibold text-[var(--text-primary)]">{title}</h2>
          <code className="min-w-0 truncate font-mono text-[11px] text-[var(--text-tertiary)]">
            {timeline.repository} @ {timeline.baseSha}
          </code>
        </div>

        <div className="ml-auto flex items-center gap-3">
          <span className="tabular-nums text-[11px] text-[var(--text-tertiary)]">
            {stepsShown}/{timeline.steps} steps
          </span>
      ...
      {replay.done && (
        <footer className="flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-[var(--border-color)] px-3 py-2 text-[11px]">
