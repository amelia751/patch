...
| Agent Registry | `agentregistry.googleapis.com` (`us-central1`). All seven agents publish an A2A card whose version and skills are derived from the `agents/config.py` grants, so the catalog cannot drift from the code. `services/github_tools` is a separate `McpServer` entry, bound to `JSONRPC` because the create operation rejects `HTTP_JSON`; that entry was created out of band rather than by `./scripts/register_agent_registry.sh`, which publishes an `McpServer` only for `patchapi-mcp-tools` and only when `PATCHAPI_MCP_URL` is set. Registry reads are fail-soft and no run branches on them. See [`packages/platform/`](./packages/platform/README.md). |
