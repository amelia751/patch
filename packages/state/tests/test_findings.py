...
def test_live_gone_beats_a_future_effective_date() -> None:
    ...
    assert (status, reason) == ("needs_you", "live_gone")


def test_live_gone_escalates_a_docs_only_hit() -> None:
    status, reason = classify(
        identifiers=["imagen-4.0-generate-001"],
        hits=[{"usage_kind": "documentation_example", "identifier": "imagen-4.0-generate-001"}],
        fail_closed=False,
...
def test_a_resolving_check_does_not_relax_a_dated_notice() -> None:
...
def test_live_gone_does_not_resurrect_a_false_positive() -> None:
    status, reason = classify(
        identifiers=["fal-ai/imagen4/preview"],
        hits=[{"usage_kind": "runtime_source", "identifier": "fal-ai/imagen4/preview"}],
        fail_closed=False,
