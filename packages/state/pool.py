...
async def create_pool(dsn: str | None = None) -> asyncpg.Pool:
    ...
    try:
        pool = await asyncpg.create_pool(
            ...
            max_inactive_connection_lifetime=IDLE_CONNECTION_SECONDS,
...
def acquire(pool: asyncpg.Pool, *, timeout: float = ACQUIRE_TIMEOUT_SECONDS) -> Any:
    """Acquire a pooled connection, or raise once the wait is no longer worth it.

    Use this anywhere a caller loops rather than serving one request.
    `Pool.acquire()` on its own waits without limit, so a pool holding only dead
    connections stops the caller instead of failing it, and a loop that never
    raises never retries and never logs.
    """
